...
var initialText = "  Atores\n    \n  Casos de Uso\n    \n  Relacionamentos\n    ";
var maskBetweenItems = ",\n    ";
var validDiagram = true;
var modified = false;

function handleUpdates() {
    document.followUpdates = document.getElementById('followUpdates').checked;
    if (document.followUpdates) { updateMessages(); }
}

function handleBeeps() {
    document.beepChanges = document.getElementById('beepChanges').checked;
}

function handleUpdateContent() {
    document.updateContent = document.getElementById('updateContent').checked;
}
 ...
 */
function createElement(elementType, elementBase, playSound) {
    play = typeof playSound !== 'undefined' ? playSound : true;
    if (!elementType)
        return false;
    ...
    if (elementBase) {   // Se recebeu elemento base, adicionar elemento a partir do mesmo
        ...
        var previousElement = "";
        // quando houver algum elemento na mesma lista, irÃ¡ tirar o ponto (fim da lista) do ultimo elemento
...
//            ucDiagram.lastRelation.end += offset;
        ...
        writeMessage("<fmt:message key='libTextual.elementCreated' />" + createdElement, 1, element.start, element.end, true, play);
        ...
        if (elementType === "ACTOR") {
            createdElement = "<fmt:message key='libTextual.typeActor' />";
            modalInputTitle.innerHTML = "<fmt:message key='libTextual.newActor' />";
        }
        else if (elementType === "USE_CASE") {
            createdElement = "<fmt:message key='libTextual.typeUseCase' />";
            modalInputTitle.innerHTML = "<fmt:message key='libTextual.newUseCase' />";
        }
 ...
 */
function createRelation(relationType, relationBase, playSound) {
    play = typeof playSound !== 'undefined' ? playSound : true;
    if (!relationType)
        return false;
    ...
    if (relationBase) { // Se origem e destino estiverem definidas, adicionar relacao a partir dos mesmos
        ...
        writeMessage("<fmt:message key='libTextual.createdRelationship' />", 1, ucDiagram.lastRelation.start, ucDiagram.lastRelation.end, true, play);
    } else {
    ...
    function getNameRelation(type) {
        switch (type) {
            case "COMMUNICATION":
                return "<fmt:message key='libTextual.association' />";
            case "INCLUSION":
                return "<fmt:message key='libTextual.inclusion' />";
            case "EXTENSION":
                return "<fmt:message key='libTextual.extension' />";
            case "GENERALIZATION":
                return "<fmt:message key='libTextual.inheritance' />";
        }
    }
}
...
if(typeof(element) !== "undefined") {
    ...
    writeMessage("<fmt:message key='libTextual.selected' />", 1, element.start, element.end);
    }
}
...
function renameElement(elementBase) {
    ...
    if (elementBase) {
        ...
        writeMessage("<fmt:message key='libTextual.renamed' />", 1, element.start, element.end);
        modified = true;
    } else {
        ...
        optionActors.innerHTML = "--<fmt:message key='libTextual.actors' />--";
...
function deleteElement(elementBase) {
    ...
    if (elementBase) {
        ...
        del(element);
        writeMessage("<fmt:message key='libTextual.deletedElement' />: " + elementText, 1);
    } else {
        ...
        var optionUseCases = document.createElement("option");
        optionUseCases.setAttribute("disabled", true);
        optionUseCases.innerHTML = "--<fmt:message key='libTextual.useCases' />--";
        ...
        optionRelations.innerHTML = "--<fmt:message key='libTextual.relationships' />--";
...
function openDiagram(diagram) {
    ucDiagram = {
        ...
        name: diagram.name || "<fmt:message key='libTextual.useCaseDiagram' />",
        actors: [],
        useCases: [],
        relations: [],
    ...
    ucDiagram.area.value = "Diagrama de Casos de Uso \"" + ucDiagram.name + "\"\n" + initialText;
    ...
    var pos = ucDiagram.name.length + 40;///25;
    ...
    ucDiagram.lastUseCase.start = ucDiagram.lastUseCase.end = pos + 21; //20;
    ...
    writeMessage("<fmt:message key='libTextual.created' />", 2, 0, 0, true, false);
    ...
    for (var i in diagram.elements) {
        ...
        createElement(diagram.elements[i].type, element, false);
    }
    for (i in diagram.relations) {
        createRelation(diagram.relations[i].type, diagram.relations[i], false);
    }
}
...
document.messages = [];
document.currentMessageIndex = 0;

function showPreviousMessage() {
//alert("previous: "+ document.currentMessageIndex);
    //alert('currentIndex = ' + document.currentMessageIndex + '; length = ' + document.messages.length);
    if(document.currentMessageIndex == 0) {
        return;
    }
    document.currentMessageIndex--;
    var mensagem = document.messages[document.currentMessageIndex];
    writeMessage(mensagem.mensagem, mensagem.tempo, mensagem.inicioSelecao, mensagem.fimSelecao, false);
}

function showNextMessage() {
//alert("previous: "+ document.currentMessageIndex);
    if(document.currentMessageIndex == document.messages.length-1) {
        return;
    }
    document.currentMessageIndex++;
    var mensagem = document.messages[document.currentMessageIndex];
 ...
 */
 var lastMessage = "initial";
function writeMessage(message, time, selectionStart, selectionEnd, gravar, playSound) {
    play = typeof playSound !== 'undefined' ? playSound : true;
    gravar = typeof gravar !== 'undefined' ? gravar : true;
    if(gravar) {
        var novaMensagem = {mensagem: message, tempo: time, inicioSelecao: selectionStart, fimSelecao: selectionEnd};
        document.messages.push(novaMensagem);
        document.currentMessageIndex = document.messages.length;
    }
    var texto = "";
    /*if(gravar) { // Seleciona o texto apenas se for a primeira vez (nao for historico), pois, no historico, as posicoes dos elementos podem ter mudado.
        setTimeout(function () {
            ...
            texto = ucDiagram.area.value.slice(selectionStart, selectionEnd)
        }, time * 1000);
    }*/
    // Se qualquer um dos dois nao for definido, posiciona o cursor no inicio de textArea
            if (!selectionStart || !selectionEnd)
            ...
            texto = ucDiagram.area.value.slice(selectionStart, selectionEnd)
                ...
                if(document.followUpdates) {
                    ...
                    if((message + ": " + texto) !== lastMessage) {
                        lastMessage = (message + ": " + texto);
                        if(play) {
                                    var speech = new SpeechSynthesisUtterance(message + ": " + texto);
                                    speech.rate = 1.5;
                                    speech.lang = 'pt-BR';
                  ...
                  //}
                    ucDiagram.messages.value = message + ": " + texto;
                    //ucDiagram.messages.focus();
                    //ucDiagram.messages.setSelectionRange(0, ucDiagram.messages.value.length);
                }
 ...
 */
function selectInterval(input, selectionStart, selectionEnd, message) {
...
function Relation() {
    ...
    Relation.prototype.init = function (id, type) {
        ...
        switch (type) {
            case "COMMUNICATION":
                this.typeString = "se associa com";
                break;
            case "EXTENSION":
                this.typeString = "estende";
                break;
            case "INCLUSION":
                this.typeString = "inclui";
                break;
            case "GENERALIZATION":
                this.typeString = "herda de";
                break;
        }
    };
}
