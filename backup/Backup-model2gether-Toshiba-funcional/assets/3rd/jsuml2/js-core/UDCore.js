...
var JSGraphic = {toPixel: function(a) {
...
Component.prototype.isOver = function(a, b) {
    if (this._visible && a >= this._x && a <= this._x + this._width && b >= this._y && b <= this._y + this._height) {
        return true
    } else {
        return false
    }
...
Component.prototype.select = function(a, b) {
    return false
};
...
TextBox.prototype.select = function(a, b) {
...
CollapsibleFields.prototype.select = function(a, b) {
    if (Math.abs(a - (this._getX() + 5)) <= 6 && Math.abs(b - (this._getY() + 5)) <= 6) {
        this.changeVisibility();
        this.notifyChange();
        return true
    } else {
        if (this.visibilitySubComponents() && Math.abs(a - (this._getX() + this.getSuperWidth() - 5)) <= 6 && Math.abs(b - (this._getY() + 5)) <= 6) {
            this.addField();
            return true
        }
    }
    return CollapsibleFields.base.select.call(this, a, b)
};
...
var AttributeItem = function(b) {
    ...
    var a = "^(?:\xAB([^\xAB\xBB:={}]+)\xBB)?([#|+|-|~])?([/])?([^\xAB\xBB:={}]+)?(?::([^\xAB\xBB:={}]+))?(?:=([^\xAB\xBB:={}]+)?)?(?:{([^\xAB\xBB:={}]+)})?$";
...
AttributeItem.prototype.showDialog = function() {
    ...
    for (d = 0; d < f.length; d++) {
        f[d].setAttribute("type", "text");
        f[d].setAttribute("value", l[d] || "")
    }
    if (l[1]) {
    ...
    var e = ["stereotype", "visibility", "derived", "name", "type", "default", "restrictions"];
    var k;
    var n;
    for (d = 0; d < f.length; d++) {
...
AttributeItem.prototype.drawShape = function(b) {
    ...
    b.lineTo(a, c + 7);
    b.lineTo(a + 10, c + 7);
    b.closePath(); /*Th e jsUML2 l ibr ary*/
    b.fill();
    b.restore()
};
...
Node.prototype.select = function(b, c) {
    if (!this._visible) {
        return false
    }
    this.deselectComponent();
    ...
    if (this._diagram._pressMouse == true) {
        if (this._selected) {
            if (this._moveable && Math.abs(b - (this._x + this._width + 2.5)) <= 5 && Math.abs(c - (this._y + this._height + 2.5)) <= 5) {
                this._resizing = true;
                return true
            }
        }
        if (this._selected) {
            if (this.isOverComponent(b, c)) {
                this._relx = b - this._x;
                this._rely = c - this._y;
        ...
        if (this._diagram._pressMouseRight == true) {
            if (this.isOver(b, c)) {
                ...
                b = b + this._diagram._div.offsetLeft;
                c = (a) ? (c - a + this._diagram._div.offsetTop) : (c + this._diagram._div.offsetTop);
                this.showContextualMenu(b, c);
                return true
            } else {
                return false
            }
...
Node.prototype.showStyleDialog = function(e) {
    ...
    var G = P._lineColor;
    var t = P._textColor;
    var ab = P._Backgroundcolor.split("#")[1];
    var g = new Array(parseInt(ab.slice(0, 2), 16), parseInt(ab.slice(2, 4), 16), parseInt(ab.slice(4, 6), 16));
    var Y = document.createElement("div");
    Y.className = "ud_popupStyle";
    var c = document.createElement("div");
    ...
    W.setAttribute("id", "divSelectColor");
    ...
    X.setAttribute("id", "divRadio");
    var T = document.createElement("label");
    T.innerHTML = "background color";
    ...
    ad.innerHTML = "text color";
    var aa = document.createElement("input");
    aa.setAttribute("id", "radio_text");
    aa.setAttribute("type", "radio");
    aa.setAttribute("name", "radio");
    aa.setAttribute("value", "text");
    ad.appendChild(aa);
    X.appendChild(L);
    X.appendChild(ad);
    X.appendChild(T);
    ...
    Q.setAttribute("style", "width: 35px");
    ...
    p.setAttribute("style", "width: 65px");
    var V = document.createElement("label");
    V.innerHTML = "font family";
    V.setAttribute("for", "family");
    var ac = document.createElement("input");
    ac.setAttribute("type", "number");
    ac.setAttribute("name", "width");
    ac.setAttribute("value", P._lineWidth || "2");
    ac.setAttribute("style", "width: 35px");
    ...
    x.setAttribute("style", "width: 85px");
    var h = document.createElement("label");
    ...
    var A = function(ae) {
        ...
        P.setLineWidth(parseFloat(ac.value, 10));
    ...
    var M = function(ae) {
        ...
        R(ag);
        ag = ag.split("#")[1];
        var af = new Array(parseInt(ag.slice(0, 2), 16), parseInt(ag.slice(2, 4), 16), parseInt(ag.slice(4, 6), 16));
        B(d, Z, af[0], "#ff0000");
        B(j, a, af[1], "#00ff00");
        B(o, f, af[2], "#0000ff")
    };
    ...
    var B = function(ag, af, ah, ae) {
        ...
        af.save();
        af.font = "12px monospace";
        af.textBaseline = "middle";
        af.fillStyle = "#ffffff";
        af.fillText(ag.getAttribute("id"), 0, ag.height / 2);
        af.restore();
        af.save();
        af.beginPath();
        af.fillStyle = ae;
        af.fillRect(20, 0, parseInt(ag.width) - 50, d.height);
        af.closePath();
        af.restore();
        af.fillStyle = "#000000";
        af.beginPath();
        af.arc(20 + (ah * 100) / 255, parseInt(ag.height) / 2, 4, 0, Math.PI * 2, true);
        af.closePath();
        af.fill();
        af.save();
        af.font = "12px monospace";
        af.textBaseline = "middle";
        af.fillStyle = "#ffffff";
        af.fillText(parseInt(ah), 125, ag.height / 2);
        af.restore()
    };
    var R = function(ae) {
        z.save();
    ...
    var J = function(ag) {
        var af = function(ak) {
            var ai = "0123456789ABCDEF";
            var ah = parseInt(ak) % 16;
            var aj = (parseInt(ak) - ah) / 16;
            hex = "" + ai.charAt(aj) + ai.charAt(ah);
            return hex
        };
        var ae = af(ag[0]) + af(ag[1]) + af(ag[2]);
        while (m.hasChildNodes()) {
            m.removeChild(m.lastChild)
        }
        m.appendChild(document.createTextNode("#"));
        m.appendChild(document.createTextNode(ae));
        C("#" + ae)
    };
    var E = function(ag) {
        var af = ag.pageX - Y.offsetLeft - this.offsetLeft;
        var ae = ag.pageY - this.offsetTop;
        if (this.getAttribute("id") == "red") {
            ...
            Z.clearRect(0, 0, parseInt(d.width), d.height);
            B(d, Z, g[0], "#ff0000")
        }
    ...
    Y.style.top = (window.innerHeight - parseInt(Y.offsetHeight)) / 2 + "px";
    Y.style.left = (window.innerWidth - parseInt(Y.offsetWidth)) / 2 + "px"
};
...
Node.prototype.isOverComponent = function(a, c) {
    var b;
    for (b = 0; b < this._components.length; b += 1) {
        if (this._components[b].isOver(a, c)) {
            return true
        }
    }
    return false
                ...
                if (this._components[d]._id == "name") {
                    if (!this._beforeNameComponent) {
                        ...
                        if (this._beforeNameComponent instanceof TextArea) {
                            this._components[d] = new TextArea({id: "name",text: h.join("\n"),text_color: g,text_family: b,font_size: a,font_weight: c,selected: e,position: Component.Bottom,margin: 3})
                        }
                        if (this._beforeNameComponent instanceof TextBox) {
                            this._components[d] = new TextBox({id: "name",text: h,text_color: g,text_family: b,font_size: a,font_weight: c,selected: e,position: Component.Bottom,margin: 3})
                        }
...
Node.prototype.showColorDialog = function(z) {
    ...
    var g = function(H) {
        var G = function(L) {
            ...
            var I = parseInt(L) % 16;
            var K = (parseInt(L) - I) / 16;
            hex = "" + J.charAt(K) + J.charAt(I);
            return hex
        };
        var F = G(H[0]) + G(H[1]) + G(H[2]);
...
InterfaceUsage.prototype.draw = function(c, a, e, d, b) {
    c.save();
    c.strokeStyle = b;
    c.translate(a, e);
    c.rotate(d);
...
Relation.prototype.setStereotype = function(a) {
    this._stereotype = new Text({id: "stereotype",text: a});
...
Relation.prototype._isOverComponent = function(a, c) {
    var b;
...
Relation.prototype.isOver = function(a, c) {
    var b;
    for (var b = 0; b < this._points.length - 1; b++) {
        if (this._selectLine(this._points[b], this._points[b + 1], a, c, 10)) {
            return true
        }
    }
    return false
};
Relation.prototype.select = function(b, d) {
    ...
    for (c = 0; c < this._points.length; c++) {
        if (Math.abs(b - this._points[c].getX()) <= 4 && Math.abs(d - this._points[c].getY()) <= 4) {
            if (this._selected > -1) {
                this._selectedBefore = true
            }
            this._selected = c;
    ...
    if (this._selected > -1) {
        if (this._isOverComponent(b, d)) {
            this._selectedBefore = true;
            return true
        }
    }
    for (var c = 0; c < this._points.length - 1; c++) {
        if (this._selectLine(this._points[c], this._points[c + 1], b, d)) {
            if (this._selected > -1) {
                this._selectedBefore = true
            }
            this._selected = c;
            this._selectedLine = true;
            this._points.splice(this._selected + 1, 0, new Point(b, d));
            return true
        }
    }
    return false
...
Relation.prototype.showStyleDialog = function(e) {
    var D = e.that || this;
    var w = D.getLineColor();
    var J = D.getLineColor().split("#")[1];
    var g = new Array(parseInt(J.slice(0, 2), 16), parseInt(J.slice(2, 4), 16), parseInt(J.slice(4, 6), 16));
    var H = document.createElement("div");
    H.className = "ud_popupLineStyle";
    var c = document.createElement("div");
    c.setAttribute("id", "divBlock1");
    var b = document.createElement("div");
    b.setAttribute("id", "divBlock2");
    ...
    G.setAttribute("id", "divSelectColor");
    ...
    z.setAttribute("id", "divLine");
    var K = document.createElement("input");
    K.setAttribute("type", "number");
    K.setAttribute("name", "width");
    K.setAttribute("value", D.getLineWidth() || "1");
    K.setAttribute("style", "width: 58px");
    ...
    q.setAttribute("name", "style");
    value = D.getLineStyle() || "solid";
    ...
    var E = document.createElement("input");
    E.setAttribute("type", "submit");
    E.setAttribute("value", "cancel");
    E.setAttribute("style", "bottom: 15px");
    var s = function(L) {
        D.setLineWidth(parseFloat(K.value, 10));
        D.setLineStyle(q.options[q.selectedIndex].value);
        D.notifyDraw();
    ...
    var A = function(L) {
        var N = D.getLineColor();
        if (!N) {
            N = "#000000"
        }
        F(N);
        N = N.split("#")[1];
        var M = new Array(parseInt(N.slice(0, 2), 16), parseInt(N.slice(2, 4), 16), parseInt(N.slice(4, 6), 16));
        t(d, I, M[0], "#ff0000");
        t(j, a, M[1], "#00ff00");
        t(n, f, M[2], "#0000ff")
    };
    ...
    var t = function(N, M, O, L) {
        ...
        M.beginPath();
        M.fillStyle = L;
        M.fillRect(20, 0, parseInt(N.width) - 50, d.height);
        M.closePath();
        M.restore();
        M.fillStyle = "#000000";
        M.beginPath();
    ...
    var v = function(N) {
        var M = N.pageX - H.offsetLeft - this.offsetLeft;
        ...
        if (this.getAttribute("id") == "red") {
            ...
            I.clearRect(0, 0, parseInt(d.width), d.height);
            t(d, I, g[0], "#ff0000")
        }
        if (this.getAttribute("id") == "green") {
            g[1] = ((M - 20) * 255) / 100;
    ...
    y.addEventListener("mousedown", v, false);
    C.addEventListener("mousedown", v, false);
    document.body.appendChild(H);
    H.style.top = (window.innerHeight - parseInt(H.offsetHeight)) / 2 + "px";
    H.style.left = (window.innerWidth - parseInt(H.offsetWidth)) / 2 + "px"
};
...
Relation.prototype.getDirection = function() {
    if (this.getDirectionToA() && this.getDirectionToB()) {
        ...
        if (this.getDirectionToA()) {
            return "a"
        } else {
            if (this.getDirectionToB()) {
                return "b"
            }
        }
    }
...
TextFields.prototype.isOver = function(a, b) {
    if (this._visible && this.visibilitySubComponents() && Math.abs(a - (this._getX() + this.getWidth() - 5)) <= 6 && Math.abs(b - (this._getY() + 5)) <= 6) {
        return true
    } else {
        return TextFields.base.isOver.call(this, a, b)
    }
};
TextFields.prototype.select = function(a, d) {
    var c = (this._orientation) ? (this._getX() + 5) : (this._getX() + this.getWidth() - 5);
    var b = (this._orientation) ? (this._getY() + this.getHeight() - 5) : (this._getY() + 5);
    if (this.visibilitySubComponents() && Math.abs(a - c) <= 6 && Math.abs(d - b) <= 6) {
        this.addField();
        return true
    } else {
        return TextFields.base.select.call(this, a, d)
    }
};
...
Region.prototype.addComponents = function(g) {
    ...
    if (this._parent._orientation) {
        ...
        if (c > 0) {
            ...
            e[c - 1].addComponent(new RegionLine({id: "region",margin: 0,width: f,height: b,position: Component.TopRight,orientation: 1}))
        }
    } else {
        if (c > 0 && g) {
        ...
        if (c > 0) {
            ...
            e[c - 1].addComponent(new RegionLine({id: "region",margin: 0,width: f,height: b,position: Component.BottomLeft,orientation: 0}))
        }
    }
};
...
TextArea.prototype.select = function(a, b) {
    if (!this._active && this.isOver(a, b)) {
        this.showDialog(a, b);
        return true
    } else {
        return false
...
SpecificationItem.prototype.showDialog = function() {
    ...
    for (e = 0; e < h.length; e++) {
        ...
        h[e].setAttribute("value", o[e] || "")
    }
    if (o[0]) {
        var l = h[0].childNodes;
        for (e = 0; e < l.length; e++) {
            if (l[e].value == k._behaviors[0]) {
                l[e].setAttribute("selected", "selected")
            }
        }
    }
    if (o[2]) {
