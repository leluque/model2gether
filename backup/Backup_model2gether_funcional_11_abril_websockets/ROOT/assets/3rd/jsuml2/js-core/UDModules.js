...
ExceptionHandler.prototype.select = function(a, c) {
    ...
    for (var b = 0; b < this._points.length - 1; b++) {
        if (this._selectLine(this._points[b], this._points[b + 1], a, c)) {
            ...
            this._points.splice(this._selected + 1, 0, new Point(a, c)); /*The jsUML2 library*/
            return true
        }
    }
    return false
        ...
        if (b != this) {
            if (this._selected == 0) {
                if (b && !(b.getType() == "UMLFork_JoinNode" && this._elemB.getType() == "UMLFork_JoinNode")) {
                    var d = this._elemA;
                    this.setElementA(b); /*The jsUML2 library*/
...
Swimlane.prototype.addComponents = function(g) {
    ...
    if (this._parent._orientation) {
        ...
        if (b > 0) {
            ...
            d[b - 1].addComponent(new SwimlaneLine({id: "region",margin: 0,width: e,height: a,position: Component.TopRight,orientation: 1}))
        }
    } else {
        if (b > 0 && g) {
        ...
        if (b > 0) {
            ...
            d[b - 1].addComponent(new SwimlaneLine({id: "region",margin: 0,width: e,height: a,position: Component.BottomLeft,orientation: 0}))
        }
    }
};
...
SwimlaneItem.prototype.createRegion = function() {
    ...
    if (this.getParent()._orientation) {
        ...
        this.getParent().addRegion(new Swimlane({parent: this.getParent(),y: this.getParent().getY() + a,x: this.getParent().getX()}))
    } else {
        if (this._parent instanceof HierarchicalSwimlane) {
        ...
        this.getParent().addRegion(new Swimlane({parent: this.getParent(),y: this.getParent()._components[b - 1]._getY(),x: this.getParent().getX() + c}))
    }
};
...
GeneralizationSet.prototype.addElement = function(a) {
    if (!(a instanceof Node)) {
        return false
    }
    for (i in this._relations) {
    ...
    relation = null;
    relation = new SetLine({a: a,b: this});
...
var SetLine = function(a) {
    a = a || {};
    SetLine.baseConstructor.call(this, a);
    f = this;
    f.setMenu([[function() {
...
SetLine.prototype.notifyChange = function() {
    if (this._points[this._selected] != this._last) {
...
SetLine.prototype._calculatePoint = function() {
    ...
    found = -1;
    splice = 1;
    var c = new Point(this._elemA.getCentralPoint());
    var d = new Point(elem.getLinkCentered(c));
    for (i in this._points) {
        if (this._points[i] == this._last) {
            found = i
        }
    }
    if (this._last == null) {
    ...
    if (found != -1 && !(d.getX() == c.getX())) {
        var b = (c.getY() - d.getY()) / (d.getX() - c.getX());
        if (b < 1 && b > -1) {
            var a = new Point(c.getX(), d.getY());
            this._points.splice(found, splice, a);
            this._last = a;
            this.notifyDraw()
        } else {
            if (found != -1) {
                var a = new Point(d.getX(), c.getY());
                this._points.splice(found, splice, a);
                this._last = a;
                this.notifyDraw()
            }
...
var UMLClassDiagram = function(b) {
    ...
    a.setValidElements(["UMLNote", "UMLLine", "UMLClass", "UMLDataType", "UMLComponent", "UMLInterfaceExtended", "UMLPackage", "UMLPackageContainer", "UMLAggregation", "UMLAssociation", "UMLComposition", "UMLDependency", "UMLGeneralization", "UMLRealization", "UMLUsage", "UMLPackageMerge", "UMLPackagePublicImport", "UMLPackagePrivateImport", "UMLGeneralizationSet", "SetLine", "UMLNAssociation", "AssociationN"]);
    return a
};
...
ComponentDiagram.prototype.addRelationFromPoints = function(c, b, l, a, g) {
    var p = this.getElementByPoint(b, l);
    var o = this.getElementByPoint(a, g);
    var n = false;
    var e;
    if (p && o) {
        if (c.getType() == "UMLGeneralization" && p instanceof Node && o instanceof Node) {
            if (p.getType() == "UMLGeneralizationSet") {
                var q = new UMLSetLine();
                q.setElements(p, o);
                q.notifyChange();
                this.addElement(q);
                n = true
            }
            for (e = 0; e < p._relations.length && !n; e++) {
                if (p._relations[e].getType() == "UMLGeneralization" && p._relations[e]._elemA == p) {
                    var d = new UMLGeneralizationSet({x: p._relations[e].getCentralPoint().getX(),y: p._relations[e].getCentralPoint().getY()});
                    d.setSuperClass(p);
                    this.addElement(d);
                    var q = new UMLSetLine();
                    q.setElements(p, d);
                    q.notifyChange();
                    this.addElement(q);
                    var m = new UMLSetLine();
                    m.setElements(d, p._relations[e]._elemB);
                    m.notifyChange();
                    this.addElement(m);
                    var h = new UMLSetLine();
                    h.setElements(d, o);
                    h.notifyChange();
                    this.addElement(h);
                    p._relations[e].remove();
                    n = true
                } else {
                    if (p._relations[e].getType() == "UMLSetLine" && p._relations[e]._elemA == p) {
                        var q = new UMLSetLine();
                        q.setElements(p._relations[e]._elemB, o);
                        this.addElement(q);
                        q.notifyChange();
                        n = true
                    }
...
NodeInterface.prototype.draw = function(b) {
    ...
    var a;
    for (a in this._relations) {
        if (this._relations[a].getType() == "UMLInterfaceRealization") {
            c = true;
            break
        }
    }
...
var UMLComponentDiagram = function(b) {
    ...
    a.setValidElements(["UMLNote", "UMLLine", "UMLComComponent", "UMLComposition", "UMLInterface", "UMLInterfaceExtended", "UMLInterfaceUsage", "UMLInterfaceRealization", "UMLPort", "UMLGeneralization", "UMLRealization", "UMLInterfaceExtended", "UMLConnector", "UMLPackage", "UMLPackageContainer", "UMLGeneralizationSet", "SetLine"]);
    return a
};
var UMLPackage = function(b) {
    var b = b || {};
...
var UMLComComponent = function(b) {
    ...
    a.addComponent(new StereotypeTagList({id: "stereotypes",centered: true}));
    a.addComponent(new TextArea({id: "name",text: "Component Name",centered: true,margin: 3}));
    a.addComponent(new AttributeFields({id: "interfaces",text: "new_interface",visibleSubComponents: false,margin: 3}));
    a.addComponent(new AttributeFields({id: "realizations",text: "new_realization",visibleSubComponents: false,margin: 3}));
    a.addComponent(new AttributeFields({id: "artifacts",text: "new_artifact",visibleSubComponents: false,margin: 3}));
    a.setMenu([[function() {
                a.showStyleDialog({that: a});
                a.removeContextualMenu()
            }, "Style"], [function() {
...
var UMLAlternative = function(b) {
    ...
    a.addComponent(new RegionAlternativeItem({id: "addRegion",text: "...",margin: 1,position: Component.BottomRight}));
    a.addFigure(new RectangleFigure());
    if (b.setElementXml) {
    } else {
...
var UMLSimpleState = function(b) {
    ...
    a.addComponent(new SpecificationItem({id: "specification",centered: false,position: Component.BottomLeft,margin: 10}));
    a.addFigure(new RoundedRectangleFigure({color: "#ffffbb"}));
    a.setMenu([[function() {
...
GeneralizationSet.prototype.addElement = function(a) {
    ...
    relation = new SetLine({a: a,b: this});
    relation._calculatePoint();
    if (this._elemB._diagram != null) {
        this._elemB._diagram.addElement(relation)
    }
    this.notifyChange();
    return true
};
...
UseCaseDiagram.prototype.addRelationFromPoints = function(c, b, l, a, g) {
    var p = this.getElementByPoint(b, l);
    var o = this.getElementByPoint(a, g);
    var n = false;
    var e;
...
var UMLUseCaseDiagram = function(b) {
    ...
    a.setValidElements(["UMLNote", "UMLLine", "UMLActor", "UMLUseCase", "UMLUseCaseExtended", "UMLSystem", "UMLSubSystem", "UMLCommunication", "UMLExtend", "UMLInclude", "UMLGeneralization", "UMLGeneralizationSet", "SetLine"]);
    return a
};
